...
export class CalendarUtilsService {
  ...
  // getWeekRangeFromToday(date = new Date()) {
  //   const today = new Date(date);
  //   const endDate = new Date(today);

  //   // trova la domenica della settimana corrente
  //   const daysToSunday = 7 - today.getDay(); // Differenza tra oggi e domenica
  //   endDate.setDate(today.getDate() + daysToSunday); // Sposta alla domenica corrente

  //   return {
  //     start: today.toISOString().split('T')[0], // Oggi
  //     end: endDate.toISOString().split('T')[0], // Domenica di questa settimana corrente
  //   };
  // }

  // hasEventsInWeek(events: iEvent[], date: Date = new Date()): boolean {
  //   // ottengo le date di inizio (il giorno corrente) e fine della settimana
  //   const { start, end } = this.getWeekRangeFromToday(date);
  //   const startDate = new Date(start);
  //   const endDate = new Date(end);

  //   // controlla se c'è almeno un'evento in questo range di date nella settimana corrente
  //   let hasEvents = events.some((event) => {
